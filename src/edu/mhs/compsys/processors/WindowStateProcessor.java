...
public class WindowStateProcessor implements IChangeProcessor {
	...
	public void process(BufferedImage img, BufferedImage img2,
		...
		if (Arrays.asList(dTbChange.getChanges()).contains(
				...
				|| Arrays.asList(dTbChange.getChanges()).contains(
						ClassificationType.TASKBAR_UPDATE_CLOSE))// IF THERE IS
																	// A
																	// TASKBAR
		...
		for (int i = 0; i < previousStateWindows.size(); i++) {
			...
			if (difBounds.getX() == 0
				...
				_changes.add(new Change(difBounds,
						ClassificationType.WINDOW_RESIZE));
			if (difBounds.size() == window.size()
					&& !difBounds.getTopLeft().equals(window.getTopLeft()))
				_changes.add(new Change(difBounds,
						ClassificationType.WINDOW_MOVE));
		}
	...
	public void proProcess(BufferedImage img1, BufferedImage img2,
		...
		for (int x = 0; x < checkWidth; x++)// total checking X area
		{
													...
													// area
			{
				if (diff.get(x, y) && notContained(x, y, areas)) {
					...
					boolean addNew = true;
					Bounds newB = new Bounds(minx, miny, maxy - miny, maxx
							- minx);
		...
		for (int i = 0; i < areas.size(); i++) {
			if (areas.get(i).getHeight() * areas.get(i).getWidth() > biggestBounds
					.getWidth() * biggestBounds.getHeight())
				biggestBounds = areas.get(i);

		}
		int minDimension = Math.min(biggestBounds.getWidth(),
				biggestBounds.getHeight());
...
=======
		if (!boundsInPrevWindow(biggestBounds, prevChanges)) {
			if (minDimension > 300) {
				if (taskbarOpen(prevChanges)
						&& taskbarChangesExist(prevChanges)
						&& !boundsInPrevWindow(biggestBounds, prevChanges)) {
					_changes.add(new Change(biggestBounds,
							ClassificationType.WINDOW_OPEN));
					if (pastIconSelect(prevChanges)) {
>>>>>>> getActiveWindow in WindowState + others + peters broken git
						addedIcon = true;
						_changes.add(new Change(pastIcon(prevChanges),
								ClassificationType.DESKTOP_ICON_CHANGE));
					}
				}
...
=======
			if (!addedIcon && minDimension > 40 && minDimension < 300
					&& !boundsInPrevWindow(biggestBounds, prevChanges)) {
				_changes.add(new Change(biggestBounds,
						ClassificationType.DESKTOP_ICON_CHANGE));
>>>>>>> getActiveWindow in WindowState + others + peters broken git
	...
	private boolean taskbarChangesExist(ArrayList<ChangeBundle> c) {
		...
		for (int i = 0; i < c.size(); i++) {
			for (int j = 0; j < c.get(i).size(); j++) {
				if (c.get(i).get(j).getType()
						.equals(ClassificationType.TASKBAR_UPDATE_CLOSE)
						|| c.get(i).get(j).getType()
								.equals(ClassificationType.TASKBAR_UPDATE_OPEN))
					ret = true;
			}
		}
		return ret;
...
=======
		// DREW
>>>>>>> took out threads because of problems
>>>>>>> getActiveWindow in WindowState + others + peters broken git
...
=======
				...
				else if (c.get(i).get(j).getType().equals(ClassificationType.WINDOW_CLOSE))
				{
					for (int w = windows.size() - 1; w >= 0; w--)
					{
						if (windows.get(w).equals(c.get(i).get(j).getBounds()))
							windows.remove(w);
					}
				}
				else if (c.get(i).get(j).getType().equals(ClassificationType.WINDOW_MOVE))
				{
					for (int w = windows.size() - 1; w >= 0; w--)
					{
						if (windows.get(w).equals(c.get(i).get(j).getBounds()))//NEEDS TO REFER TO OLD WINDOW, NOT YET DONE 
							windows.remove(w);
					}
				}
				else if (c.get(i).get(j).getType().equals(ClassificationType.WINDOW_RESIZE))
				{
//					for (int w = windows.size() - 1; w >= 0; w--) //What to do for window resize?  remove the window bounds of one before?
//					{
//						if (windows.get(w).equals(c.get(i).get(j).getBounds())) 
//							windows.remove(w - 1);
//					}
...
=======
		for (int i = 0; i < c.size(); i++) {
			for (int j = 0; j < c.get(i).size(); j++) {
				if (c.get(i).get(j).getType()
						.equals(ClassificationType.DESKTOP_ICON_CHANGE)) {
					if (!found) {
>>>>>>> getActiveWindow in WindowState + others + peters broken git
	...
	private Bounds getActiveWindow(BinaryImage diff, ArrayList<Bounds> windows,
			ArrayList<ChangeBundle> c) {
		Bounds[] windowChange = new Bounds[windows.size()];
		...
		if (windows.size() > 0) {
			...
			for (int i = 0; i < windowChange.length; i++) {
				...
				int k = c.size() - 1;
				while (maxChange.size() == windowChange[i].size() && k >= 0) {
					for (int j = c.get(k).size() - 1; j > 0; j--) {
						Bounds prevChange = c.get(k).get(j).getBounds();
						if (BoundsProcessor.intersection(prevChange,
								windows.get(window)).size() < BoundsProcessor
								.intersection(prevChange, windows.get(i))
								.size()) {
							maxChange = windowChange[i];
							window = i;
						}
	...
	public ArrayList<Change> getPROChanges() {
		if (_changes == null) {
			System.out
					.println(this.getClass() + ".getPROChanges returned null");
